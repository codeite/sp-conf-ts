...
export function readString(
  name: string | string[],
  suppliedOptions?: Partial<Options<string>> | string,
): string | undefined {
  const options = _cleanOptions(
    typeof suppliedOptions === 'string' ? { defaultValue: suppliedOptions } : suppliedOptions,
  );
  ...
  if (val !== undefined) {
    if (_failedvalidator(options.validator, val)) {
      if (!options.isInnerFunction) {
        options.error(
          `Expected env var "${name}" to be match pattern "${options.validator}" but was "${val}" and did not.`,
        );
        missingEnvVars = true;
      }
    } else {
...
export function readNumber(
  name: string | string[],
  suppliedOptions?: Partial<Options<number>> | number,
): number | undefined {
  const options = _cleanOptions(
    typeof suppliedOptions === 'number' ? { defaultValue: suppliedOptions } : suppliedOptions,
  );
...
export function readBoolean(
  name: string | string[],
  suppliedOptions?: Partial<BooleanOptions> | boolean,
): boolean | undefined {
  ...
  const extraTrueValues = optionsObj?.trueValue
    ? Array.isArray(optionsObj.trueValue)
      ? optionsObj.trueValue
      : [optionsObj.trueValue.toLocaleLowerCase()]
    : [];
  const extraFalseValues = optionsObj?.falseValue
    ? Array.isArray(optionsObj.falseValue)
      ? optionsObj.falseValue
      : [optionsObj.falseValue.toLocaleLowerCase()]
    : [];
...
export function readCertificate(
  name: string | string[],
  suppliedOptions?: Partial<CertificateOptions> | string,
): string | undefined {
  ...
  if (val !== undefined) {
    ...
    options.log(
      `Using default ${name} ${obfuscateCertificate(
        options.defaultValue,
        options.beginCertificate,
...
export function readUrl(
  ...
  if (val !== undefined) {
    if (_failedvalidator(options.validator, val)) {
      if (!options.isInnerFunction) {
        options.error(
          `Expected env var "${name}" to be match pattern "${options.validator}" but was "${obfuscateAuth(
            val,
          )}" and did not.`,
        );
        missingEnvVars = true;
